...
the main component to the complexity is the priority queue Q,
an efficent implementation of Q uses a heap, this allows us to get the min value in O(logn), this key update can be done through a bit of added code complexity and features in the priority queue, also being in O(logn) time,
